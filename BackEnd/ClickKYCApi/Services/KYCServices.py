...
from .CallClickKycSPServices import sp_initialize_tracking_id, sp_get_tracking_data, sp_update_tracking_data

async def InitializeTrackingId(phone_number: str, product_code: str) -> str:
    db_session = None
    try:
        db_session = AsyncSessionLocalClickKyc()
        return await sp_initialize_tracking_id(db_session, phone_number, product_code)
    except Exception as ex:
        error_msg = f"{str(ex)}\n{traceback.format_exc()}"
        await AddLogOrError(SystemLogErrorSchema(
            Msg=error_msg,
            Type = "ERROR",
            ModuleName = "KYCServices/InitializeTrackingId",
            CreatedBy = ""
        ))
        return None
    finally:
